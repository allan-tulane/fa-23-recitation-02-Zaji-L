...
**The code shows that when f(n) = 1, we observe linear growth of O(n). When f(n) = log(n) we observe nlogn growth O(nlogn). When f(n) = we observe linear growth of O(n)**
...
** If c<log_b a the behavior is similar to reccurence with a smaller exponent and W(n) grows at a slow rate. If c>log_b a is the opposite of the previous case. When they are equal, there is an equal amount of polynomial and exponetial growth.**
